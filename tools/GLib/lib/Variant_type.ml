...
let _new =
foreign "g_variant_type_new" (ptr t_typ @-> string @-> returning (ptr (* interface *) t_typ))

let new_array =
foreign "g_variant_type_new_array" (ptr t_typ @-> ptr (* interface *) t_typ @-> returning (ptr (* interface *) t_typ))

let new_dict_entry =
foreign "g_variant_type_new_dict_entry" (ptr t_typ @-> ptr (* interface *) t_typ @-> ptr (* interface *) t_typ @-> returning (ptr (* interface *) t_typ))

let new_maybe =
foreign "g_variant_type_new_maybe" (ptr t_typ @-> ptr (* interface *) t_typ @-> returning (ptr (* interface *) t_typ))

(*Not implemented g_variant_type_new_tuple argument types not handled*)
let copy =
foreign "g_variant_type_copy" (ptr t_typ @-> returning (ptr (* interface *) t_typ))

let dup_string =
foreign "g_variant_type_dup_string" (ptr t_typ @-> returning (string))

let element =
foreign "g_variant_type_element" (ptr t_typ @-> returning (ptr (* interface *) t_typ))

let equal =
foreign "g_variant_type_equal" (ptr t_typ @-> ptr (* interface *) t_typ @-> returning (bool))

let first =
foreign "g_variant_type_first" (ptr t_typ @-> returning (ptr (* interface *) t_typ))
...
let is_subtype_of =
foreign "g_variant_type_is_subtype_of" (ptr t_typ @-> ptr (* interface *) t_typ @-> returning (bool))
...
let key =
foreign "g_variant_type_key" (ptr t_typ @-> returning (ptr (* interface *) t_typ))

let n_items =
foreign "g_variant_type_n_items" (ptr t_typ @-> returning (uint64_t))

let next =
foreign "g_variant_type_next" (ptr t_typ @-> returning (ptr (* interface *) t_typ))

let value =
foreign "g_variant_type_value" (ptr t_typ @-> returning (ptr (* interface *) t_typ))

let checked_ =
foreign "g_variant_type_checked_" (ptr t_typ @-> string @-> returning (ptr (* interface *) t_typ))
