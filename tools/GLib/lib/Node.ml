...
let f_next = field t_typ "next" (ptr (* interface *) Node.t_typ)
let f_prev = field t_typ "prev" (ptr (* interface *) Node.t_typ)
let f_parent = field t_typ "parent" (ptr (* interface *) Node.t_typ)
let f_children = field t_typ "children" (ptr (* interface *) Node.t_typ)
let child_index =
foreign "g_node_child_index" (ptr t_typ @-> ptr_opt void @-> returning (int32_t))

let child_position =
foreign "g_node_child_position" (ptr t_typ @-> ptr (* interface *) t_typ @-> returning (int32_t))
...
let is_ancestor =
foreign "g_node_is_ancestor" (ptr t_typ @-> ptr (* interface *) t_typ @-> returning (bool))
...
let n_nodes =
foreign "g_node_n_nodes" (ptr t_typ @-> (* interface *) Traverse_flags.t_list_view @-> returning (uint32_t))
