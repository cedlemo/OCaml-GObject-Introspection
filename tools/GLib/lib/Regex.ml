...
let _new =
foreign "g_regex_new" (ptr t_typ @-> string @-> (* interface *) Regex_compile_flags.t_list_view @-> (* interface *) Regex_match_flags.t_list_view  @-> ptr_opt (ptr Error.t_typ) @-> returning (ptr_opt (* interface *) t_typ))

let get_capture_count =
foreign "g_regex_get_capture_count" (ptr t_typ @-> returning (int32_t))

let get_compile_flags =
foreign "g_regex_get_compile_flags" (ptr t_typ @-> returning ((* interface *) Regex_compile_flags.t_list_view))

let get_has_cr_or_lf =
foreign "g_regex_get_has_cr_or_lf" (ptr t_typ @-> returning (bool))

let get_match_flags =
foreign "g_regex_get_match_flags" (ptr t_typ @-> returning ((* interface *) Regex_match_flags.t_list_view))
...
let ref =
foreign "g_regex_ref" (ptr t_typ @-> returning (ptr (* interface *) t_typ))
...
let match_simple =
foreign "g_regex_match_simple" (ptr t_typ @-> string @-> string @-> (* interface *) Regex_compile_flags.t_list_view @-> (* interface *) Regex_match_flags.t_list_view @-> returning (bool))
