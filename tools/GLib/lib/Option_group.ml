...
let add_entries =
foreign "g_option_group_add_entries" (ptr t_typ @-> ptr (* interface *) Option_entry.t_typ @-> returning (void))

let free =
foreign "g_option_group_free" (ptr t_typ @-> returning (void))

let ref =
foreign "g_option_group_ref" (ptr t_typ @-> returning (ptr (* interface *) t_typ))
