...
let get_function_info namespace name =
  ...
  | Some base_info -> (
      ...
      | Function ->
          let info = Function_info.from_baseinfo base_info in
          Some info
      | _ -> None)
  ...
  | Some base_info -> (
      ...
      | Interface -> (
          ...
          | Some info' ->
              ...
              assert_bool "It is a method"
                (List.mem Bindings.Function_info.Is_method flags);
              assert_bool "Throws"
                (List.mem Bindings.Function_info.Throws flags)
          | None ->
              assert_equal_string "Gio::File#set_attribute" "should exists")
      | baseinfo_t ->
          assert_equal_string "Gio::File should be an Interface instead of "
            (Bindings.Base_info.string_of_info_type baseinfo_t))
