...
let get_type_info () =
  ...
  | None ->
      assert_equal_string fn_name "No base info found";
      None
  | Some base_info -> (
...
let test_get_tag _ =
  type_test (fun info ->
      ...
      assert_equal
        ~printer:(fun tag -> Bindings.Types.string_of_tag tag)
...
let test_get_array_type _ =
  type_test (fun info ->
      ...
      | Some array_type ->
          assert_equal
            ~printer:(fun t -> Bindings.Types.string_of_array_type t)
            C array_type)
  ...
  | Some base_info -> (
      ...
      | Function -> (
          ...
          | Some interface -> (
              ...
              | Some interface_name ->
                  assert_equal_string "TypeClass" interface_name))
