...
let test_baseinfo_get_type _ =
  ...
  | Some base_info ->
      assert_equal_boolean true
        (match Base_info.get_type base_info with Struct -> true | _ -> false)
        ...
        | f' :: q ->
            let _ =
              assert_equal
                ...
                Bindings.Field_info.Is_readable f'
            in
            check_flags q
      in
      check_flags flags)
...
let test_get_field_out_of_bounds _ =
  struct_test (fun info ->
      try ignore (Struct_info.get_field info 300) with
      | Failure message ->
          assert_equal_string "Array Index out of bounds" message
...
let test_get_method_out_of_bounds _ =
  struct_test (fun info ->
      try ignore (Struct_info.get_method info 300) with
      | Failure message ->
          assert_equal_string "Array Index out of bounds" message
      | _ -> assert_equal_string "Bad exception" "Not a Failure")
...
let test_find_method _ =
  struct_test (fun info ->
      match Struct_info.find_method info "copy" with
      | None -> assert_equal_boolean false true
      | Some fn_info ->
          let symbol = Function_info.get_symbol fn_info in
          assert_equal_string "g_value_copy" symbol)
...
let test_find_method_bad_name _ =
  struct_test (fun info ->
      ...
      | Some _ ->
          assert_equal_string "Impossible" "Should not returns something")
...
let tests =
  ...
  >::: [
         ...
         "Struct_info get method out of bounds"
         >:: test_get_method_out_of_bounds;
