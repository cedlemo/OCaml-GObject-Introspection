...
let test_from_baseinfo _ =
  ...
  | Some base_info ->
      assert_equal_boolean true
        (match Base_info.get_type base_info with Union -> true | _ -> false)
...
let test_get_field_out_of_bounds _ =
  union_test (fun info ->
      try ignore (Union_info.get_field info 3000) with
      | Failure message ->
...
let test_get_method_out_of_bounds _ =
  union_test (fun info ->
      try ignore (Union_info.get_method info 300) with
      | Failure message ->
          assert_equal_string "Array Index out of bounds" message
      | _ -> assert_equal_string "Bad exception" "Not a Failure")
