...
open Foreign
type t
let t_typ : t union typ = union "TokenValue"
let f_v_symbol = field t_typ "v_symbol" (ptr void)
let f_v_identifier = field t_typ "v_identifier" (ptr string)
...
let f_v_string = field t_typ "v_string" (ptr string)
let f_v_comment = field t_typ "v_comment" (ptr string)
