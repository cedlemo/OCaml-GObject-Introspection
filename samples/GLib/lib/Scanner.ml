...
let f_token = field t_typ "token" (Core.token_type)
...
let f_next_token = field t_typ "next_token" (Core.token_type)
(* TODO Struct field Scanner : union tag not implemented . *)
...
let cur_token =
foreign "g_scanner_cur_token" (ptr t_typ @-> returning (Core.token_type))
let destroy =
foreign "g_scanner_destroy" (ptr t_typ @-> returning (void))
let eof =
foreign "g_scanner_eof" (ptr t_typ @-> returning (bool))
let get_next_token =
foreign "g_scanner_get_next_token" (ptr t_typ @-> returning (Core.token_type))
let input_file =
...
let peek_next_token =
foreign "g_scanner_peek_next_token" (ptr t_typ @-> returning (Core.token_type))
...
let unexp_token =
foreign "g_scanner_unexp_token" (ptr t_typ @-> Core.token_type @-> string @-> string @-> string @-> string @-> int32_t @-> returning (void))
