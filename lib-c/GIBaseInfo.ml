...
(** Obtain the name of the info . What the name represents depends on the
    ...
    the function. *)
external get_name:
  baseinfo -> string = "caml_g_base_info_get_name_c"

(** Obtain the info type of the GIBaseInfo *)
external get_type:
  baseinfo -> baseinfo_type = "caml_g_base_info_get_type_c"
...
(** Compare two GIBaseInfo.  Using pointer comparison is not practical since
    ...
    comparisons. *)
external equal:
  baseinfo -> baseinfo -> bool = "caml_g_base_info_equal_c"

(** Obtain the namespace of info .*)
external get_namespace:
  baseinfo -> string = "caml_g_base_info_get_namespace_c"

(** Iterate over all attributes associated with this node. *)
external iterate_over_attributes:
  baseinfo -> (string -> string -> bool) -> unit = "caml_g_base_info_iterate_over_attributes_c"
