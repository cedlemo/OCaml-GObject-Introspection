...
let get_closure =
  foreign "g_arg_info_get_closure" (ptr arginfo @-> returning int)

let get_destroy =
  foreign "g_arg_info_get_destroy" (ptr arginfo @-> returning int)
...
let may_be_null =
  foreign "g_arg_info_may_be_null" (ptr arginfo @-> returning bool)

let is_caller_allocates =
  foreign "g_arg_info_is_caller_allocates" (ptr arginfo @-> returning bool)

let is_optional =
  foreign "g_arg_info_is_optional" (ptr arginfo @-> returning bool)

let is_return_value =
  foreign "g_arg_info_is_return_value" (ptr arginfo @-> returning bool)

let is_skip = foreign "g_arg_info_is_skip" (ptr arginfo @-> returning bool)
...
let get_type info =
  let get_type_raw =
    foreign "g_arg_info_get_type"
      (ptr arginfo @-> returning (ptr Type_info.typeinfo))
  in
  let info' = get_type_raw info in
  Type_info.add_unref_finaliser info'

(* TODO : check that the info can be casted to arg info ? *)
let cast_from_baseinfo info = coerce (ptr Base_info.baseinfo) (ptr arginfo) info
let cast_to_baseinfo info = coerce (ptr arginfo) (ptr Base_info.baseinfo) info
