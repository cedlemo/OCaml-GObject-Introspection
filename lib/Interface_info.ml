...
let get_prerequisite info n =
  let get_prerequisite_raw =
    foreign "g_interface_info_get_prerequisite"
      (ptr interfaceinfo @-> int @-> returning (ptr Base_info.baseinfo))
  in
  let max = get_n_prerequisites info in
  if n < 0 || n >= max then raise (Failure "Array Index out of bounds")
  else
    let info' = get_prerequisite_raw info n in
...
let get_property info n =
  let get_property_raw =
    foreign "g_interface_info_get_property"
      (ptr interfaceinfo @-> int @-> returning (ptr Property_info.propertyinfo))
  in
...
let get_n_methods =
  foreign "g_interface_info_get_n_methods" (ptr interfaceinfo @-> returning int)

let get_method info n =
  let get_method_raw =
    foreign "g_interface_info_get_method"
      (ptr interfaceinfo @-> int @-> returning (ptr Function_info.functioninfo))
  in
...
let get_n_signals =
  foreign "g_interface_info_get_n_signals" (ptr interfaceinfo @-> returning int)

let get_signal info n =
  let get_signal_raw =
    foreign "g_interface_info_get_signal"
      (ptr interfaceinfo @-> int @-> returning (ptr Signal_info.signalinfo))
  in
...
let find_signal info name =
  let find_signal_raw =
    foreign "g_interface_info_find_signal"
      (ptr interfaceinfo @-> string
      @-> returning (ptr_opt Signal_info.signalinfo))
  in
...
let get_constant info n =
  let get_constant_raw =
    foreign "g_interface_info_get_constant"
      (ptr interfaceinfo @-> int @-> returning (ptr Constant_info.constantinfo))
  in
...
let get_iface_struct info =
  let get_iface_struct_raw =
    foreign "g_interface_info_get_iface_struct"
      (ptr interfaceinfo @-> returning (ptr_opt Struct_info.structinfo))
  in
...
let get_n_vfuncs =
  foreign "g_interface_info_get_n_vfuncs" (ptr interfaceinfo @-> returning int)

let get_vfunc info n =
  let get_vfunc_raw =
    foreign "g_interface_info_get_vfunc"
      (ptr interfaceinfo @-> int @-> returning (ptr VFunc_info.vfuncinfo))
  in
...
let find_vfunc info name =
  let find_vfunc_raw =
    foreign "g_interface_info_find_vfunc"
      (ptr interfaceinfo @-> string @-> returning (ptr_opt VFunc_info.vfuncinfo))
  in
