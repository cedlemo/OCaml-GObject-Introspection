...
let is_gtype_struct =
  foreign "g_struct_info_is_gtype_struct" (ptr structinfo @-> returning bool)

let get_alignment =
  foreign "g_struct_info_get_alignment" (ptr structinfo @-> returning int)

let get_size =
  foreign "g_struct_info_get_size" (ptr structinfo @-> returning int)

let is_foreign =
  foreign "g_struct_info_is_foreign" (ptr structinfo @-> returning bool)

let get_n_fields =
  foreign "g_struct_info_get_n_fields" (ptr structinfo @-> returning int)

let get_n_methods =
  foreign "g_struct_info_get_n_methods" (ptr structinfo @-> returning int)

let get_field info n =
  let get_field_raw =
    foreign "g_struct_info_get_field"
      (ptr structinfo @-> int @-> returning (ptr Field_info.fieldinfo))
  in
...
let get_method info n =
  let get_method_raw =
    foreign "g_struct_info_get_method"
      (ptr structinfo @-> int @-> returning (ptr Function_info.functioninfo))
  in
