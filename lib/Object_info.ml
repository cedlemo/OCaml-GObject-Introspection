...
let get_abstract =
  foreign "g_object_info_get_abstract" (ptr objectinfo @-> returning bool)

let get_fundamental =
  foreign "g_object_info_get_fundamental" (ptr objectinfo @-> returning bool)

let get_parent info =
  let get_parent_raw =
    foreign "g_object_info_get_parent"
      (ptr objectinfo @-> returning (ptr Base_info.baseinfo))
  in
  let info' = get_parent_raw info in
  Base_info.add_unref_finaliser info'

let get_type_name =
  foreign "g_object_info_get_type_name" (ptr objectinfo @-> returning string)

let get_type_init =
  foreign "g_object_info_get_type_init" (ptr objectinfo @-> returning string)

let get_n_constants =
  foreign "g_object_info_get_n_constants" (ptr objectinfo @-> returning int)

let get_constant info n =
  let get_constant_raw =
    foreign "g_object_info_get_constant"
      (ptr objectinfo @-> int @-> returning (ptr Constant_info.constantinfo))
  in
  let max = get_n_constants info in
  if n < 0 || n >= max then raise (Failure "Array Index out of bounds")
  else
    let info' = get_constant_raw info n in
    Constant_info.add_unref_finaliser info'

let get_n_fields =
  foreign "g_object_info_get_n_fields" (ptr objectinfo @-> returning int)

let get_field info n =
  let get_field_raw =
    foreign "g_object_info_get_field"
      (ptr objectinfo @-> int @-> returning (ptr Field_info.fieldinfo))
  in
...
let get_n_interfaces =
  foreign "g_object_info_get_n_interfaces" (ptr objectinfo @-> returning int)

let get_interface info n =
  let get_interface_raw =
    foreign "g_object_info_get_interface"
      (ptr objectinfo @-> int @-> returning (ptr Interface_info.interfaceinfo))
  in
  let max = get_n_interfaces info in
  if n < 0 || n >= max then raise (Failure "Array Index out of bounds")
  else
    let info' = get_interface_raw info n in
    Interface_info.add_unref_finaliser info'

let get_n_methods =
  foreign "g_object_info_get_n_methods" (ptr objectinfo @-> returning int)

let get_method info n =
  let get_method_raw =
    foreign "g_object_info_get_method"
      (ptr objectinfo @-> int @-> returning (ptr Function_info.functioninfo))
  in
...
let find_method info name =
  let find_method_raw =
    foreign "g_object_info_find_method"
      (ptr objectinfo @-> string
      @-> returning (ptr_opt Function_info.functioninfo))
  in
  match find_method_raw info name with
  | None -> None
  | Some info' ->
      let info'' = Function_info.add_unref_finaliser info' in
      Some info''

let get_n_properties =
  foreign "g_object_info_get_n_properties" (ptr objectinfo @-> returning int)

let get_property info n =
  let get_property_raw =
    foreign "g_object_info_get_property"
      (ptr objectinfo @-> int @-> returning (ptr Property_info.propertyinfo))
  in
  let max = get_n_properties info in
  if n < 0 || n >= max then raise (Failure "Array Index out of bounds")
  else
    let info' = get_property_raw info n in
    Property_info.add_unref_finaliser info'

let get_n_signals =
  foreign "g_object_info_get_n_signals" (ptr objectinfo @-> returning int)

let get_signal info n =
  let get_signal_raw =
    foreign "g_object_info_get_signal"
      (ptr objectinfo @-> int @-> returning (ptr Signal_info.signalinfo))
  in
  let max = get_n_signals info in
  if n < 0 || n >= max then raise (Failure "Array Index out of bounds")
  else
    let info' = get_signal_raw info n in
    Signal_info.add_unref_finaliser info'

let find_signal info name =
  let find_signal_raw =
    foreign "g_object_info_find_signal"
      (ptr objectinfo @-> string @-> returning (ptr_opt Signal_info.signalinfo))
  in
  match find_signal_raw info name with
  | None -> None
  | Some info' ->
      let info'' = Signal_info.add_unref_finaliser info' in
      Some info''

let get_n_vfuncs =
  foreign "g_object_info_get_n_vfuncs" (ptr objectinfo @-> returning int)

let get_vfunc info n =
  let get_vfunc_raw =
    foreign "g_object_info_get_vfunc"
      (ptr objectinfo @-> int @-> returning (ptr VFunc_info.vfuncinfo))
  in
  let max = get_n_vfuncs info in
  if n < 0 || n >= max then raise (Failure "Array Index out of bounds")
  else
    let info' = get_vfunc_raw info n in
    VFunc_info.add_unref_finaliser info'

let find_vfunc info name =
  let find_vfunc_raw =
    foreign "g_object_info_find_vfunc"
      (ptr objectinfo @-> string @-> returning (ptr_opt VFunc_info.vfuncinfo))
  in
  match find_vfunc_raw info name with
  | None -> None
  | Some info' ->
      let info'' = VFunc_info.add_unref_finaliser info' in
      Some info''

let get_class_struct info =
  let get_class_struct_raw =
    foreign "g_object_info_get_class_struct"
      (ptr objectinfo @-> returning (ptr_opt Struct_info.structinfo))
  in
  match get_class_struct_raw info with
  | None -> None
  | Some info' ->
      let info'' = Struct_info.add_unref_finaliser info' in
      Some info''
...
let find_method_using_interfaces info name =
  ...
  | Some info' -> (
      ( Some info',
        match
          coerce (ptr objectinfo) (ptr_opt objectinfo) !@implementor_addr
        with
        | None -> None
...
let find_vfunc_using_interfaces info name =
  let find_vfunc_using_interfaces_raw =
    foreign "g_object_info_find_vfunc_using_interfaces"
      (ptr objectinfo @-> string
      @-> ptr (ptr objectinfo)
      @-> returning (ptr_opt VFunc_info.vfuncinfo))
  in
  let implementor_addr = allocate_n (ptr objectinfo) ~count:1 in
