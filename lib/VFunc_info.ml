...
let get_offset =
  foreign "g_vfunc_info_get_offset" (ptr vfuncinfo @-> returning int)

let get_signal info =
  let get_signal_raw =
    foreign "g_vfunc_info_get_signal"
      (ptr vfuncinfo @-> returning (ptr_opt Callable_info.callableinfo))
  in
...
let get_flags =
  foreign "g_vfunc_info_get_flags" (ptr vfuncinfo @-> returning flags_list)

(* TODO : check that the info can be casted to vfunc info ? *)
let cast_from_baseinfo info =
  coerce (ptr Base_info.baseinfo) (ptr vfuncinfo) info

let cast_to_baseinfo info = coerce (ptr vfuncinfo) (ptr Base_info.baseinfo) info
